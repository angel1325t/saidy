...
function getCopyLabel(copy: CopyOption) {
  return copy.copy_code || copy.barcode || copy.id.slice(0, 8);
}
...
export function LibraryOperationsPanel({
    ...
    const load = async () => {
      ...
      try {
        ...
        if (active) {
          ...
          setError(loadError instanceof Error ? loadError.message : 'No se pudo cargar la operacion bibliotecaria');
        }
      } finally {
  ...
  const runAction = async (label: string, action: () => Promise<void>, successMessage = 'Accion registrada correctamente.') => {
    setSubmitting(label);
    setError(null);
    setMessage(null);
    try {
  ...
  const handleArchiveMaterial = (materialId: string) => {
    ...
    }, 'Material archivado correctamente.');
  };
  ...
  const handleCreateAcquisition = (event: FormEvent<HTMLFormElement>) => {
    ...
    }, 'Solicitud de adquisicion creada.');
  };
  ...
  const handleCreateInterlibrary = (event: FormEvent<HTMLFormElement>) => {
    ...
    }, 'Solicitud interbibliotecaria creada.');
  };
  ...
  const handleStatusPatch = (path: string, status: string, successMessage: string) => {
  ...
  return (
    <section className={showTabs ? 'admin-rbac library-ops' : 'admin-rbac library-ops library-ops--embedded'}>
      <div className="panel__header">
        <div>
          <span className="panel__eyebrow">Operacion</span>
          <h2>Catalogo, circulacion y administracion bibliotecaria</h2>
        </div>
        {showTabs ? (
          <div className="admin-rbac__tabs">
            ...
            <button
              type="button"
              className={activeTab === 'circulation' ? 'is-active' : ''}
      ...
      {loading ? <div className="empty-state">Cargando modulos operativos...</div> : null}
        ...
        <div className="admin-rbac__layout">
          <article className="panel editor-card">
            <div className="panel__header">
              <div>
                ...
                <h3>Alta bibliografica</h3>
              </div>
              <span className="badge">{materials.length} materiales</span>
            </div>
            <form className="auth-form crud-form crud-form--catalog" onSubmit={handleCreateMaterial}>
              <label>
                Titulo
                <input value={materialForm.title} onChange={(event) => setMaterialForm((current) => ({ ...current, title: event.target.value }))} required />
              </label>
              <label>
                Tipo
                <select value={materialForm.kind} onChange={(event) => setMaterialForm((current) => ({ ...current, kind: event.target.value }))}>
                  {materialKinds.map((kind) => (
                    <option key={kind} value={kind}>
                      {formatMaterialKind(kind)}
                    </option>
                  ))}
                </select>
              </label>
              <label>
                Autores
                <input value={materialForm.contributors} onChange={(event) => setMaterialForm((current) => ({ ...current, contributors: event.target.value }))} placeholder="Autor 1, Autor 2" />
              </label>
              <label>
                Etiquetas
                <input value={materialForm.tags} onChange={(event) => setMaterialForm((current) => ({ ...current, tags: event.target.value }))} placeholder="ciencia, tesis" />
              </label>
              <label>
                Editorial
                <input value={materialForm.publisher} onChange={(event) => setMaterialForm((current) => ({ ...current, publisher: event.target.value }))} />
              </label>
              <label>
                Ano
                <input type="number" value={materialForm.publication_year} onChange={(event) => setMaterialForm((current) => ({ ...current, publication_year: event.target.value }))} />
              </label>
              <label>
                ISBN
                <input value={materialForm.isbn} onChange={(event) => setMaterialForm((current) => ({ ...current, isbn: event.target.value }))} />
              </label>
              <label>
                ISSN
                <input value={materialForm.issn} onChange={(event) => setMaterialForm((current) => ({ ...current, issn: event.target.value }))} />
              </label>
              <label>
                DOI
                <input value={materialForm.doi} onChange={(event) => setMaterialForm((current) => ({ ...current, doi: event.target.value }))} />
              </label>
              <label>
                Dewey
                <input value={materialForm.dewey_classification} onChange={(event) => setMaterialForm((current) => ({ ...current, dewey_classification: event.target.value }))} />
              </label>
              <label>
                Signatura
                <input value={materialForm.call_number} onChange={(event) => setMaterialForm((current) => ({ ...current, call_number: event.target.value }))} />
              </label>
              <label>
                URL digital
                <input type="url" value={materialForm.digital_url} onChange={(event) => setMaterialForm((current) => ({ ...current, digital_url: event.target.value }))} />
              </label>
              <label>
                Resumen
                <textarea rows={4} value={materialForm.summary} onChange={(event) => setMaterialForm((current) => ({ ...current, summary: event.target.value }))} />
              </label>
              <button type="submit" disabled={submitting === 'material'}>
                {submitting === 'material' ? 'Guardando...' : 'Crear material'}
              </button>
            </form>
          </article>
          ...
          <article className="panel panel--wide">
            <div className="panel__header">
              ...
              <span className="badge">{copies.length} copias</span>
            </div>
            ...
            <form className="auth-form crud-form crud-form--copy" onSubmit={handleCreateCopy}>
              ...
              <label>
                Codigo
                <input value={copyForm.copy_code} onChange={(event) => setCopyForm((current) => ({ ...current, copy_code: event.target.value }))} />
              </label>
              <label>
                Barcode
              ...
              <label>
                Ubicacion
                <input value={copyForm.location} onChange={(event) => setCopyForm((current) => ({ ...current, location: event.target.value }))} />
              </label>
              <label>
                Estante
                <input value={copyForm.shelf_code} onChange={(event) => setCopyForm((current) => ({ ...current, shelf_code: event.target.value }))} />
              </label>
              <label>
                Estado
                <select value={copyForm.status} onChange={(event) => setCopyForm((current) => ({ ...current, status: event.target.value }))}>
                  {['available', 'borrowed', 'reserved', 'lost', 'maintenance', 'archived'].map((status) => (
              ...
              <label>
                Condicion
                <textarea rows={2} value={copyForm.condition_note} onChange={(event) => setCopyForm((current) => ({ ...current, condition_note: event.target.value }))} />
              </label>
              <button type="submit" disabled={submitting === 'copy'}>
                {submitting === 'copy' ? 'Guardando...' : 'Agregar copia'}
              </button>
            </form>

            {materials.length === 0 ? (
              <div className="empty-state">No hay materiales cargados en el catalogo.</div>
            ) : (
              <div className="data-table-shell">
                <table className="data-table">
                  ...
                  <tbody>
                    {materials.map((material) => {
                      const materialCopies = copies.filter((copy) => copy.material_id === material.id);
                      return (
                        <tr key={material.id}>
                          <td>
                            <div className="table-primary">
                              <strong>{material.title}</strong>
                              <span>{material.id.slice(0, 8)}</span>
                            </div>
                          </td>
                          <td>{formatMaterialKind(material.kind)}</td>
                          <td>{material.status || 'published'}</td>
                          <td>
                            <div className="badge-row badge-row--table">
                              {materialCopies.length > 0 ? (
                                materialCopies.map((copy) => (
                                  <button
                                    key={copy.id}
                                    type="button"
                                    className="badge"
                                    onClick={() => handleCopyStatus(copy.id, copy.status === 'available' ? 'maintenance' : 'available')}
                                  >
                                    {getCopyLabel(copy)} · {copy.status}
                                  </button>
                                ))
                              ) : (
                                <span className="table-muted">Sin copias</span>
                              )}
                            </div>
                          </td>
                          <td>
                            <div className="table-actions">
                              ...
                              <button type="button" onClick={() => handleArchiveMaterial(material.id)}>
                                Archivar
                              </button>
                            </div>
                          </td>
        ...
        <div className="admin-rbac__layout">
          {canManageLoans ? (
            <article className="panel editor-card">
              <div className="panel__header">
                <div>
                  <span className="panel__eyebrow">Prestamo</span>
                  <h3>Registrar circulacion</h3>
                </div>
              </div>
              <form className="auth-form crud-form" onSubmit={handleCreateLoan}>
                <label>
                  Usuario
                  <select value={loanForm.user_id} onChange={(event) => setLoanForm((current) => ({ ...current, user_id: event.target.value }))}>
                    <option value="">Yo / sesion activa</option>
                    {users.map((user) => (
                ...
                <label>
                  Material
                  <select
                    value={loanForm.material_id}
                    onChange={(event) => setLoanForm((current) => ({ ...current, material_id: event.target.value, copy_id: '' }))}
                  >
                    {materials.map((material) => (
                      <option key={material.id} value={material.id}>
                        {material.title}
                ...
                <label>
                  Tipo
                  <select value={loanForm.loan_type} onChange={(event) => setLoanForm((current) => ({ ...current, loan_type: event.target.value }))}>
                    <option value="physical">Fisico</option>
                    <option value="digital">Digital</option>
                  </select>
                </label>
                {loanForm.loan_type === 'physical' ? (
                  <label>
                    Copia
                    <select value={loanForm.copy_id} onChange={(event) => setLoanForm((current) => ({ ...current, copy_id: event.target.value }))}>
                      <option value="">Seleccionar copia</option>
                      {availableCopies.map((copy) => (
                        <option key={copy.id} value={copy.id}>
                          {getCopyLabel(copy)} · {copy.location || 'Sin ubicacion'}
                        </option>
                      ))}
                    </select>
                  </label>
                ) : null}
                <button type="submit" disabled={submitting === 'loan'}>
                  {submitting === 'loan' ? 'Registrando...' : 'Crear prestamo'}
                </button>
              </form>
            </article>
          ) : null}
          ...
          <article className="panel panel--wide">
            <div className="panel__header">
              <div>
                ...
                <h3>Prestamos, reservas y multas</h3>
              </div>
            </div>
            <div className="admin-grid admin-grid--triple">
              <div className="info-card info-card--stacked">
                <strong>Prestamos</strong>
                {loans.length === 0 ? (
                  <div className="table-muted">Sin prestamos registrados.</div>
                ) : (
                  <div className="mini-list mini-list--wide">
                    {loans.map((loan) => (
                      <article key={loan.id}>
                        <strong>{loan.materials?.title || loan.id}</strong>
                        <span>{loan.status} · vence {formatDate(loan.due_at)}</span>
                        <div className="badge-row">
                          <button
                            type="button"
                            className="badge"
                            onClick={() =>
                              runAction(`renew-${loan.id}`, async () => {
                                await apiFetch(`/api/circulation/loans/${loan.id}/renew`, token, { method: 'POST' });
                              }, 'Prestamo renovado.')
                            }
                          >
                            Renovar
                          ...
                          <button
                            ...
                            className="badge"
                            onClick={() =>
                              runAction(`return-${loan.id}`, async () => {
                                await apiFetch(`/api/circulation/loans/${loan.id}/return`, token, { method: 'POST' });
                              }, 'Prestamo devuelto.')
                            }
                          >
                            Devolver
              ...
              <div className="info-card info-card--stacked">
                ...
                {reservations.length === 0 ? (
                  <div className="table-muted">Sin reservas registradas.</div>
                ) : (
                  <div className="mini-list mini-list--wide">
                    {reservations.map((reservation) => (
                      <article key={reservation.id}>
                        <strong>{reservation.materials?.title || reservation.id}</strong>
                        <span>{reservation.status}</span>
                        <button
                          type="button"
                          className="badge"
                          onClick={() =>
                            runAction(`reservation-${reservation.id}`, async () => {
                              await apiFetch(`/api/circulation/reservations/${reservation.id}/cancel`, token, { method: 'POST' });
                            }, 'Reserva cancelada.')
                          }
                        >
                          Cancelar
              ...
              <div className="info-card info-card--stacked">
                ...
                {fines.length === 0 ? (
                  <div className="table-muted">Sin multas registradas.</div>
                ) : (
                  <div className="mini-list mini-list--wide">
                    {fines.map((fine) => (
                      <article key={fine.id}>
                        ...
                        <span>{fine.status} · {fine.reason || 'Sin detalle'}</span>
                        <button
                          type="button"
                          className="badge"
                          onClick={() =>
                            runAction(`fine-${fine.id}`, async () => {
                              await apiFetch(`/api/circulation/fines/${fine.id}/pay`, token, { method: 'POST' });
      ...
      {activeTab === 'admin' ? (
        <div className="admin-rbac__layout admin-rbac__layout--stacked">
            ...
            <article className="panel editor-card admin-rbac__form-card">
              <div className="panel__header">
                <div>
                  ...
                  <h3>Auditoria fisica</h3>
                </div>
              </div>
              <form className="auth-form crud-form" onSubmit={handleCreateInventory}>
                <label>
                  Material
                  <select value={inventoryForm.material_id} onChange={(event) => setInventoryForm((current) => ({ ...current, material_id: event.target.value }))}>
                    {materials.map((material) => (
                      <option key={material.id} value={material.id}>
                        {material.title}
                      </option>
                ...
                <label>
                  Copia
                  <select value={inventoryForm.copy_id} onChange={(event) => setInventoryForm((current) => ({ ...current, copy_id: event.target.value }))}>
                    <option value="">Sin copia especifica</option>
                    {copies.map((copy) => (
                      <option key={copy.id} value={copy.id}>
                        {getCopyLabel(copy)}
                      </option>
                ...
                <label>
                  Condicion
                  <input value={inventoryForm.condition_status} onChange={(event) => setInventoryForm((current) => ({ ...current, condition_status: event.target.value }))} />
                </label>
                <label className="toggle-row">
                  <input
                    type="checkbox"
                    checked={inventoryForm.is_missing}
                    onChange={(event) => setInventoryForm((current) => ({ ...current, is_missing: event.target.checked }))}
                  />
                  <span>Marcar como perdido</span>
                </label>
                <label>
                  Notas
                  <textarea rows={2} value={inventoryForm.notes} onChange={(event) => setInventoryForm((current) => ({ ...current, notes: event.target.value }))} />
                </label>
                <button type="submit" disabled={submitting === 'inventory'}>
                  {submitting === 'inventory' ? 'Guardando...' : 'Registrar inventario'}
                </button>
              </form>
            </article>
          ) : null}
          ...
          <article className="panel admin-rbac__content">
            <div className="panel__header">
              <div>
                <span className="panel__eyebrow">Gestion</span>
                <h3>Adquisiciones, interbiblioteca, avisos y analitica</h3>
              </div>
            </div>
            ...
            <div className="admin-grid admin-grid--double">
              {canManageReports ? (
                <form className="auth-form info-card crud-form" onSubmit={handleCreateAcquisition}>
                  <strong>Solicitud de adquisicion</strong>
                  <input placeholder="Titulo" value={acquisitionForm.title} onChange={(event) => setAcquisitionForm((current) => ({ ...current, title: event.target.value }))} required />
                  <select value={acquisitionForm.kind} onChange={(event) => setAcquisitionForm((current) => ({ ...current, kind: event.target.value }))}>
                    {materialKinds.map((kind) => (
                      <option key={kind} value={kind}>
                        {formatMaterialKind(kind)}
                      </option>
                  ...
                  <textarea rows={2} placeholder="Justificacion" value={acquisitionForm.justification} onChange={(event) => setAcquisitionForm((current) => ({ ...current, justification: event.target.value }))} />
                  <button type="submit" disabled={submitting === 'acquisition'}>
                    {submitting === 'acquisition' ? 'Guardando...' : 'Crear adquisicion'}
                  </button>
                </form>
              ) : null}
              ...
              {canManageReports ? (
                <form className="auth-form info-card crud-form" onSubmit={handleCreateInterlibrary}>
                  <strong>Prestamo interbibliotecario</strong>
                  ...
                  <button type="submit" disabled={submitting === 'interlibrary'}>
                    {submitting === 'interlibrary' ? 'Guardando...' : 'Crear solicitud'}
                  </button>
                </form>
              ) : null}
              ...
              {canManageNotifications ? (
                <form className="auth-form info-card crud-form" onSubmit={handleCreateNotification}>
                  <strong>Notificacion interna</strong>
                  <select value={notificationForm.user_id} onChange={(event) => setNotificationForm((current) => ({ ...current, user_id: event.target.value }))}>
                  ...
                  <input placeholder="Titulo" value={notificationForm.title} onChange={(event) => setNotificationForm((current) => ({ ...current, title: event.target.value }))} required />
                  ...
                  <button type="submit" disabled={submitting === 'notification'}>
                    {submitting === 'notification' ? 'Enviando...' : 'Enviar aviso'}
                  </button>
                </form>
              ) : null}

              {analytics?.summary ? (
                <article className="info-card info-card--stacked">
                  <strong>Analitica operativa</strong>
                  <p>Prestamos activos: {analytics.summary.active_loans}</p>
                ...
                <tbody>
                    ...
                    <tr key={record.id}>
                      <td>
                        <div className="table-primary">
                          ...
                          <span>{record.supplier || record.external_library || 'Sin origen'}</span>
                        </div>
                      </td>
                      <td>{'title' in record ? 'Adquisicion' : 'Interbiblioteca'}</td>
                      <td>{record.status}</td>
                      <td>
                        {'title' in record ? (
                          <div className="table-actions table-actions--inline">
                            <button type="button" onClick={() => handleStatusPatch(`/api/admin/acquisitions/${record.id}`, 'approved', 'Adquisicion aprobada.')}>
                              Aprobar
                            </button>
                            <button type="button" className="secondary" onClick={() => handleStatusPatch(`/api/admin/acquisitions/${record.id}`, 'received', 'Adquisicion recibida.')}>
                              Recibido
                            </button>
                          </div>
                        ) : (
                          <div className="table-actions table-actions--inline">
                            <button type="button" onClick={() => handleStatusPatch(`/api/admin/interlibrary/${record.id}`, 'approved', 'Solicitud aprobada.')}>
                              Aprobar
                            </button>
                            <button type="button" className="secondary" onClick={() => handleStatusPatch(`/api/admin/interlibrary/${record.id}`, 'returned', 'Material devuelto.')}>
                              Devuelto
                            </button>
                          </div>
                        )}
                  ...
                  {inventory.slice(0, 4).map((item) => (
                    <tr key={item.id}>
                      <td>
                        <div className="table-primary">
                          <strong>{item.materials?.title || item.copy_id || item.id}</strong>
                          <span>{item.location || 'Sin ubicacion'}</span>
                        </div>
                      </td>
                      <td>Inventario</td>
                      <td>{item.is_missing ? 'perdido' : item.condition_status}</td>
                      <td><span className="table-muted">Registro informativo</span></td>
                    </tr>
                  ))}
                  {notifications.slice(0, 4).map((item) => (
                    <tr key={item.id}>
                      ...
                      <td>Notificacion</td>
                      <td>{item.read_at ? 'leida' : 'pendiente'}</td>
                      <td><span className="table-muted">Sin accion directa</span></td>
                    </tr>
                  ))}
                </tbody>
              </table>
